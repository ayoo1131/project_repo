...
def check_highest_user_id(user_id, username):
    ...
    if not highest_id: #If there are no users in user table, add user id to id_tracker table
        new_user_id = Id_Tracker(username = username, id=user_id)
        db.session.add(new_user_id)
        db.session.commit()
        return user_id

    elif highest_id.id >= user_id: #If the highest id is equal to new user id, then highest user's been deleted. Update id_tracker and user tables
        highest_id_value = highest_id.id

        highest_id.username=username
        highest_id.id = (highest_id_value + 1)
    ...
    elif highest_id.id < user_id: #Record new highest user id in id_tracker
        highest_id.username = username
        highest_id.id = user_id
        db.session.commit()
        return user_id
